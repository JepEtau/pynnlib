...
from .att_modules import Frequency_Edge_Module
...
class EfficientNet(nn.Module):
    def __init__(
        ...
        global_params=None,
        arch: int = 7,
        frequency_radius: int = 16,
        gamma: float = 0.1,
    ):
        raise NotImplementedError("missing frequency radius/gamma")
        super().__init__()
    ...
    def get_blocks(self, x: Tensor, H, W):
        # Blocks
        for idx, block in enumerate(self._blocks):
            drop_connect_rate = self._global_params.drop_connect_rate
            if drop_connect_rate:
