...
class Frequency_Edge_Module(nn.Module):
    def __init__(self, radius, channel, gamma: float = 0.1):
        super(Frequency_Edge_Module, self).__init__()
        self.radius = radius
        self.UAM = UnionAttentionModule(channel, only_channel_tracing=True, gamma=gamma)
    ...
    def mask_radial(self, img: Tensor, r):
...
class UnionAttentionModule(nn.Module):
    def __init__(self, n_channels, only_channel_tracing = False, gamma: float = 0.1):
    ...
    def masking(self, x: Tensor, mask: Tensor):
    ...
    def Channel_Tracer(self, x: Tensor):
        avg_pool = self.GAP(x)
    ...
    def forward(self, x):
        ...
        q: Tensor = self.spatial_q(x_drop).squeeze(1)
        k: Tensor = self.spatial_k(x_drop).squeeze(1)
        v: Tensor = self.spatial_v(x_drop).squeeze(1)
...
class aggregation(nn.Module):
    def __init__(self, channel, gamma: float = 0.1):
        ...
        self.UAM = UnionAttentionModule(channel[0] + channel[1] + channel[2], gamma=gamma)
...
class ObjectAttention(nn.Module):
    def __init__(self, channel, kernel_size, denoise: float = 0.93):
    ...
    def forward(self, decoder_map, encoder_map):
        ...
        x: Tensor = mask_ob.expand(-1, self.channel, -1, -1).mul(encoder_map)
