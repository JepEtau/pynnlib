...
class NnLib:
    ...
    def convert_to_tensorrt(self,
        ...
        if model.fwk_type == NnFrameworkType.PYTORCH:
            arch: NnArchitecture = model.arch
            if arch.to_tensorrt is None:
                raise ValueError(red(f"[E]: Conversion to tensorRT: arch \'{arch.name}\' is not supported"))
            if dtype not in arch.to_tensorrt.dtypes:
